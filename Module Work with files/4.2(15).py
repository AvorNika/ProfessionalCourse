# функция, возвращающая словарь, ключами которого являются названия столбцов таблицы, а значениями - содержимое этих столбцов
def csv_columns(filename):
    with open(filename, encoding='utf-8') as file:
        import csv
        data = csv.DictReader(file)
        keys_from_table = data.fieldnames
        result = dict.fromkeys(keys_from_table, [])
        for line in data:
            for k in keys_from_table:
                result[k] = result.get(k, []) + [line[k]]
